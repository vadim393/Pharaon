...
public class BindCommand extends Command {
    ...
    private void handleAddBind(IArgConsumer args) throws CommandException {
        args.requireMin(2);
        String rest = normalizeSpaces(args.rawRest());
        int split = rest.lastIndexOf(' ');
        if (split <= 0) {
            logDirect("Укажи модуль и клавишу. Пример: bind add Elytra Motion R", Formatting.GRAY);
            return;
        }

        String moduleName = normalizeSpaces(rest.substring(0, split));
        String keyName = rest.substring(split + 1).trim().toUpperCase(Locale.ROOT);
        ...
        int key = KeyStorage.getKey(keyName);
        if (key == -1) {
    ...
    private void handleRemoveBind(IArgConsumer args) throws CommandException {
        args.requireMin(1);
        String moduleName = normalizeSpaces(args.rawRest());
        Module module = moduleStorage.get(moduleName);
        if (module == null) {
            logDirect(Formatting.GRAY + "Модуль с названием " + Formatting.WHITE + moduleName + Formatting.GRAY + " не найден");
            return;
    ...
    public Stream<String> tabComplete(String label, IArgConsumer args) throws CommandException {
        ...
        String action = raw.get(0).getValue().toLowerCase(Locale.ROOT);

        if (action.equals("add")) {
            // If all args except the last one form an exact module name, complete key names.
            if (size >= 3) {
                String moduleCandidate = normalizeSpaces(joinArguments(raw, 1, size - 1));
                if (moduleStorage.get(moduleCandidate) != null) {
                    String keyPrefix = raw.get(size - 1).getValue();
            ...
            // Otherwise complete module name as a full phrase (supports spaces).
            String modulePrefix = normalizeSpaces(joinArguments(raw, 1, size));
    ...
    private static String joinArguments(List<ICommandArgument> args, int fromInclusive, int toExclusive) {
