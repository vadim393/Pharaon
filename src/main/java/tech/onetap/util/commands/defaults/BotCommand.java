...
public class BotCommand extends Command {
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        switch (action) {
            ...
            case "list" -> list();
            default -> logDirect("Неизвестная подкоманда. Используй add/connect/remove/return/control/say/sayall/list");
        }
    }
    ...
    private void add(IArgConsumer args) throws CommandException {
        ...
        String server = args.getString();
        String proxy = args.getString();
        try {
            BotSessionManager.connectViaProxy(name, server, proxy);
            logDirect("Подключение через прокси выполнено: " + name + " -> " + server + " через " + proxy);
    ...
    public Stream<String> tabComplete(String label, IArgConsumer args) throws CommandException {
        ...
        if (args.hasExactly(2)) {
            ...
            if (action.equals("add")) return Stream.of("<ник_бота>");
            if (action.equals("connect")) return Stream.of("<ник_бота>");
            if (action.equals("sayall")) return Stream.of("<сообщение>");
        }

        if (args.hasExactly(3) && args.peekString(0).equalsIgnoreCase("add")) {
            return Stream.of("<сервер>");
        }

        if (args.hasExactly(4) && args.peekString(0).equalsIgnoreCase("add")) {
            return Stream.of("<айпи:порт_прокси>");
        }
    ...
    public List<String> getLongDesc() {
        return List.of(
                ...
                "",
                "Использование:",
                "> bot add <name> <server> <proxy_ip:port>",
                "> bot connect <name> <ip>",
                "> bot control <name>",
