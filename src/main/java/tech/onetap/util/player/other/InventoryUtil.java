...
public class InventoryUtil implements IMinecraft {
    ...
    public static int inventoryToScreenSlot(int inventorySlot) {
        if (inventorySlot >= 9 && inventorySlot <= 35) return inventorySlot;
        if (inventorySlot >= 0 && inventorySlot <= 8) return 36 + inventorySlot;
        return -1;
    }

    public static int findBestHotbarSlotForItem(Item item) {
        if (mc.player == null) return -1;

        int selectedSlot = mc.player.getInventory().selectedSlot;
        ItemStack selectedStack = mc.player.getInventory().getStack(selectedSlot);
        if (selectedStack.isEmpty() || selectedStack.isOf(item)) {
            return selectedSlot;
        }
    ...
    public static int moveInventoryItemToHotbar(Item item, int hotbarSlot) {
        if (mc.player == null || mc.interactionManager == null || hotbarSlot < 0 || hotbarSlot > 8) {
            return -1;
        }

        int inventorySlot = searchItem(item, 9, 36);
        if (inventorySlot == -1) {
            return -1;
        }
        ...
        clickSlotNoSync(mc.player.currentScreenHandler.syncId, screenSlot, hotbarSlot, SlotActionType.SWAP, mc.player);
        mc.player.currentScreenHandler.sendContentUpdates();
        return hotbarSlot;
    }

    public static int moveInventorySlotToHotbar(int inventorySlot, int hotbarSlot) {
        if (mc.player == null || mc.interactionManager == null || hotbarSlot < 0 || hotbarSlot > 8) {
            return -1;
        }
    ...
    public static int ensureHotbarItem(Item item, int preferredHotbarSlot) {
        int hotbarSlot = searchItemHotbar(item);
        if (hotbarSlot != -1) {
            return hotbarSlot;
        }

        int targetSlot = preferredHotbarSlot >= 0 && preferredHotbarSlot <= 8
                ? preferredHotbarSlot
                : findBestHotbarSlotForItem(item);

        return moveInventoryItemToHotbar(item, targetSlot);
    }
