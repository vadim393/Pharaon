...
public class CrystalDamageCalculator implements IMinecraft {
    public float calculateCrystalDamage(Entity crystal, LivingEntity target, double extraResistance) {
        return calculateCrystalDamage(new Vec3d(crystal.getX(), crystal.getY(), crystal.getZ()), target, extraResistance);
    }

    public float calculateCrystalDamage(Vec3d crystalPos, LivingEntity target, double extraResistance) {
        if (mc.player.getOffHandStack().getItem() != Items.PLAYER_HEAD) extraResistance = 0;

        double explosionRadius = 12.0;
        double dx = target.getX() - crystalPos.x;
        double dy = target.getEyeY() - crystalPos.y;
        double dz = target.getZ() - crystalPos.z;
        double distance = Math.sqrt(dx * dx + dy * dy + dz * dz) / explosionRadius;
        if (distance > 1.0) return 0.0F;

        float exposure = ExplosionImpl.calculateReceivedDamage(crystalPos, target);

        double impact = (1.0 - distance) * exposure;
        float rawDamage = (float) (((impact * impact + impact) * (mc.player.getY() <= crystalPos.y ? 3.7 : 3.35f)) * 7 * 2.0);
