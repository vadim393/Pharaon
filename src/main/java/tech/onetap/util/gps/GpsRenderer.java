...
public final class GpsRenderer implements IMinecraft {
    ...
    private void onHud(EventHUD e) {
        ...
        float triangleSize = 50.0f;
        float half = triangleSize * 0.5f;

        BuiltTexture arrow2 = Builder.texture()
                .size(new SizeState(triangleSize, triangleSize))
                .radius(QuadRadiusState.NO_ROUND)
                .color(new QuadColorState(theme))
                .texture(0, 0, 1, 1, id)
                .smoothness(2)
                .build();
        arrow2.render(ms.peek().getPositionMatrix(), -half, -half);
        ...
        BuiltTexture arrow = Builder.texture()
                ...
                .smoothness(1f)
                .build();
        arrow.render(ms.peek().getPositionMatrix(), -half, -half);
        ms.pop();
