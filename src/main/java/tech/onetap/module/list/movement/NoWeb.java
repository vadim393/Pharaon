...
public class NoWeb extends Module {
    private static volatile boolean breakThroughActive;
    private static volatile boolean ignoreCobwebHitboxActive;

    private final BooleanSetting breakThrough = new BooleanSetting("Ломать через", false);
    private final BindSetting ignoreCobwebBind = new BindSetting("Игнор паутины", -1);

    public static boolean isBreakThroughActive() {
        return breakThroughActive;
    }

    public static boolean isIgnoreCobwebHitboxActive() {
        return ignoreCobwebHitboxActive;
    }

    @Override
    public void onEnable() {
        super.onEnable();
        syncBreakThroughState();
        ignoreCobwebHitboxActive = false;
    }
    ...
    private void onKey(EventKeyInput event) {
        if (!isEnabled() || event.getAction() != 1 || mc.currentScreen != null) {
            return;
        }

        if (ignoreCobwebBind.getValue() != -1 && event.getKey() == ignoreCobwebBind.getValue()) {
            ignoreCobwebHitboxActive = !ignoreCobwebHitboxActive;
        }
    }
    ...
    private void syncBreakThroughState() {
        breakThroughActive = isEnabled() && breakThrough.getValue();
    }

    @Subscribe
    private void onCobweb(EventCobweb e) {
        if (ignoreCobwebHitboxActive) {
            return;
        }
