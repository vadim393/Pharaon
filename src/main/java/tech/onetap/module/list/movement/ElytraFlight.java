...
@ModuleInformation(moduleName = "ElytraFlight", moduleDesc = "флай элики.", moduleCategory = ModuleCategory.MOVEMENT)
public class ElytraFlight extends Module {

    private final SliderSetting fireworkDelay = new SliderSetting("Задержка феера", 750, 100, 2000, 50);
    private final ModeSetting antiKick = new ModeSetting("AntiKick", "Jitter", "Jitter", "Glide");
    private static final float TARGET_BPS = 33.0f;
    private static final float BASE_HORIZONTAL_SPEED = TARGET_BPS / 20.0f;
    ...
    private void onUpdate(EventPlayerUpdate e) {
        ...
        if (!killauraActive && isMoving) {
            float motion = (pitch == -25 || pitch == 25) ? BASE_HORIZONTAL_SPEED * 1f : (pitch == 90 || pitch == -90) ? BASE_HORIZONTAL_SPEED * 0.25f : BASE_HORIZONTAL_SPEED;
            double rad = Math.toRadians(yaw + 90);
            double motionX = Math.cos(rad) * motion;
            double motionZ = Math.sin(rad) * motion;
            double vy = getAntiKickY();
            if (pitch == -25) vy = 0.5;
            ...
            mc.player.setVelocity(0, getAntiKickY(), 0);
        }
    ...
    private double getAntiKickY() {
        return antiKick.is("Glide")
                ? -0.08
                : (mc.player.age % 2 == 0 ? 0.08 : -0.08);
    }
}
