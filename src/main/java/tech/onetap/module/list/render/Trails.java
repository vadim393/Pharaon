...
public class Trails extends Module {
    ...
    public final ModeSetting mode = new ModeSetting("Вид", "Default", "Default", "BreadCrumbs");
    private final SliderSetting duration = new SliderSetting("Длительность", 1000.0f, 500.0f, 2000.0f, 100.0f).setVisible(() -> mode.is("Default"));

    private final SliderSetting lineWidth = new SliderSetting("Толщина", 2.0f, 0.5f, 5.0f, 0.1f).setVisible(() -> mode.is("Default"));
    private final SliderSetting opacity = new SliderSetting("Непрозрачность", 100.0f, 0.0f, 255.0f, 5.0f).setVisible(() -> mode.is("Default"));

    private final SliderSetting limit = new SliderSetting("Лимит точек", 1000.0f, 10.0f, 5000.0f, 10.0f).setVisible(() -> mode.is("BreadCrumbs"));
    private final SliderSetting lifetime = new SliderSetting("Время жизни (сек)", 30.0f, 1.0f, 300.0f, 1.0f).setVisible(() -> mode.is("BreadCrumbs"));
    private final BooleanSetting throughWalls = new BooleanSetting("Сквозь стены", true).setVisible(() -> mode.is("BreadCrumbs"));
    ...
    private void onRender(WorldRenderContext context) {
        ...
        if (mode.is("BreadCrumbs")) {
            updateBreadCrumbs(mc, currentTime);
        }
        ...
        for (Map.Entry<Integer, List<Point>> entry : entityPoints.entrySet()) {
            ...
            if (entity != null && entity.isAlive()) {
                ...
                if (mode.is("Default")) {
                    renderTrail(points, context, currentTime, getTrailHeight(entity), entityType);
                } else if (mode.is("BreadCrumbs") && entry.getKey() == mc.player.getId()) {
                    renderBreadCrumbs(points, context, currentTime);
                }
            }
        }
    }

    private void updateBreadCrumbs(MinecraftClient mc, long currentTime) {
        List<Point> points = entityPoints.computeIfAbsent(mc.player.getId(), k -> new ArrayList<>());

        long lifetimeMs = (long) (lifetime.getValue() * 1000.0f);
        points.removeIf(pos -> currentTime - pos.timestamp >= lifetimeMs);

        while (points.size() > (int) limit.getValue()) {
            points.remove(points.size() - 1);
        }

        Vec3d feetPos = mc.player.getPos();
        Vec3d lastPos = lastPositions.get(mc.player.getId());

        if (lastPos == null || lastPos.distanceTo(feetPos) > 0.01) {
            points.add(0, new Point(feetPos, currentTime));
            lastPositions.put(mc.player.getId(), feetPos);
        }
    }

    private void renderBreadCrumbs(List<Point> points, WorldRenderContext context, long currentTime) {
        ...
        for (int i = 1; i < points.size(); i++) {
            ...
            int color1 = tech.onetap.module.settings.impl.ThemeManager.getInstance().getCurrentTheme().getColorTheme(0);
            int color2 = tech.onetap.module.settings.impl.ThemeManager.getInstance().getCurrentTheme().getColorTheme(100);
            float progress = (float) i / (float) points.size();
            int colorInt = ColorProvider.interpolate(color1, color2, progress);

            float alpha = 1.0f;
            if (i < 10) {
                alpha = (float) i / 10.0f;
            }
            colorInt = ColorProvider.setAlpha(colorInt, (int) (alpha * 255));

            buffer.vertex(matrix, (float) prev.pos.x, (float) prev.pos.y, (float) prev.pos.z).color(colorInt);
            buffer.vertex(matrix, (float) curr.pos.x, (float) curr.pos.y, (float) curr.pos.z).color(colorInt);
        }
    ...
    private int getColorForEntity(EntityType entityType, int index) {
        if (entityType == EntityType.SELF || entityType == EntityType.FRIEND || entityType == null) {
            return tech.onetap.module.settings.impl.ThemeManager.getInstance().getCurrentTheme().getColorTheme(index * 2);
        }
    ...
    private float getTrailHeight(Entity entity) {
        float height = entity.getHeight();
        if (entity instanceof PlayerEntity player && WorldTweaks.shouldRenderSmallPlayer(player)) {
            return Math.max(0.35f, height * 0.5f);
        }
        return height;
    }
